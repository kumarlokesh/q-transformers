...
class AdvancedMemoryProfiler:
    ...
    def get_peak_memory(self) -> Dict[str, float]:
        ...
        return {
            ...
            "peak_torch_allocated_mb": max(
                s.torch_allocated_mb for s in self.snapshots
            ),
            "peak_torch_cached_mb": max(s.torch_cached_mb for s in self.snapshots),
            "peak_python_tracemalloc_mb": max(
                s.python_tracemalloc_mb for s in self.snapshots
            ),
        }
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "efficiency_ratio": (
                    baseline_memory / current_memory if current_memory > 0 else 0
                ),
            },
...
@contextmanager
def memory_profile_attention(
    attention_fn, *args, profiler_name: str = "attention", **kwargs
):
    """
...
_MemoryProfiler = AdvancedMemoryProfiler
